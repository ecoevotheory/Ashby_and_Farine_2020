...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
    ...
    plhs[1] = mxCreateDoubleMatrix(colLen, 2, mxREAL);
    T = mxGetPr(plhs[0]);
    x = mxGetPr(plhs[1]);
...
int my_rungkut (double *T, double *x_out, double *EQFLAG, double *init_pop, struct PARAM *p){
    ...
    double x[2], dxdt[2], x_scale[2], xmin[2], xmax[2], hnext[1], h[1], t, nextcheck;
...
void rkqs(double *x, double *dxdt, double *h, double *hnext, double *x_scale, struct PARAM *p)
{
    double x_temp[2], x_err[2], htemp, errmax;
    int i, j, count;
...
void rkck(double *x, double *dxdt, double *xout, double *xerr, double h, struct PARAM *p){
    int i, j;
    double xk1[2], xk2[2], xk3[2], xk4[2], xk5[2], xk6[2], xtemp[2];
